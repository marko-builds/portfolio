...
# The mark on its own, for anywhere that supplies its own background. The source's comment
# block is replaced rather than inherited — it says "THIS FILE IS THE SOURCE", which is false
# in a derived file and is exactly the kind of copied claim that outlives its truth.
awk '
  /<!--/ { skip = 1; print "  <!-- Generated by scripts/render-icons.sh from favicon.svg. Do not edit."
           print "       The mark with no tile behind it, for surfaces that bring their own background."
           print "       Geometry and rationale live in favicon.svg, which is the source. -->" }
  /-->/  { skip = 0; next }
  !skip  { print }
' "$SRC" | sed '/<rect id="tile"/d' > "$PUB/logo.svg"
# Asserts on `<rect`, not on the id the sed above matches: a guard that shares its pattern
# with the mutation it checks passes whenever a rename breaks both, which is no guard at all.
n=$(command grep -c '<rect' "$PUB/logo.svg" || true)
[ "$n" = 0 ] || { echo "tile strip left $n rect(s) behind" >&2; exit 1; }
n=$(command grep -c 'THIS FILE IS THE SOURCE' "$PUB/logo.svg" || true)
[ "$n" = 0 ] || { echo "logo.svg inherited the source's comment" >&2; exit 1; }
n=$(command grep -c '<path' "$PUB/logo.svg" || true)
[ "$n" = 1 ] || { echo "logo.svg has $n chevron path(s), expected 1" >&2; exit 1; }
