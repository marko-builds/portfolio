...
opener: "Instantiate and Destroy are the obvious way to spawn obstacles in a Unity endless runner, and the reliable way to get a frame spike every time a chunk scrolls into view. This entry is the dig through the profiler, the Queue-based object pool and static PoolManager that flattened the allocation line, and the trade-offs that came with the design."
