...
opener: "Habitagram's progress screen is one mountain ridge climbing from sea to summit, rendered as a single panorama and cut into six swipeable pages. Continuity across every cut is the whole point, and nearly every bug in the feature was some way of breaking it. The sea ate the desert, the phone's crop manufactured seams the art never had, and each one left a rule behind."
...
And the reached-versus-locked split depends on the user's current tier (how far up the ridge their streak has taken them), which arrives from an async query. Render before it resolves and the frontier defaults to zero, flashing the user's own past biomes grey for a frame. The sibling [journey screen](/field-journal/deterministic-journey-art/) already gated on the loading state; the new screen didn't, until it did. When you copy a pattern to a new screen, copy its load-gate too.

The engine's output is locked by [golden-fingerprint](/field-journal/golden-fingerprints-generative-art/) tests, all twelve tiles hashed at the approved seed, so the render is pinned to the one I signed off on a real device.
