...
opener: "The enemy in my Hide & Seek prototype lived in one Update() method, a chain of if/else that grew until two branches shared a variable and the Chase to Patrol transition started misfiring. This entry replaces that loop with a generic IState and StateMachine pattern in Unity, and records what it taught me about GC allocation, coupling and designing for change."
...
When the enemy's Chase to Patrol transition started misfiring because two branches shared a variable, I knew the update loop had to go.
...
When I added the Search state, the Chase to Patrol transition started firing incorrectly because both branches read from the same `_lastKnownPosition` variable. Fixing one broke the other. The root cause wasn't the length of the function. It was shared mutable state with no clear ownership.
