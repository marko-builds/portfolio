...
The stack is four engines in one folder. `music.py`, `sfx.py`, a set of scenery renderers behind one orchestrator, and `vfx.py` for overlay loops. No samples, no models, no downloads. If a sound exists in the output, a formula in the repo produced it.
