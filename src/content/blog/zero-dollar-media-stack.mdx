...
![Three panels. Left: a spectrogram of the synthesized zap, showing a bank of falling diagonal lines as the saw sweep drops from 1900 Hz to 200 Hz, with a horizontal ripple from the 75 Hz buzz. Middle: the waveform envelope of four bars of the lofi loop, two either side of the loop point, where the wrap is marked in amber and looks like the ordinary bar lines marked in grey. Right: a single frame of the caustics scenery loop, pale blue light filaments on a dark blue field.](/images/blog/zero-dollar-media-stack/synth-engines.png)
