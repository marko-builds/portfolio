...
The game I am building has tooling around it: headless Blender exporters, an Unreal import plugin, the validation that rejects a bad asset before it reaches the engine. I write that tooling with Claude Code, and the model writes a lot of the code. The reason the tools are reliable is not the model. It is the loop I put around it.
